...
public class DowsingRod extends Item {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        ...
        list.add(new TextComponentTranslation("text.oredowsing.tooltip.0",
                (target_stack != null ? target_stack.getDisplayName()
                        : new TextComponentTranslation("text.oredowsing.all_ores").getUnformattedComponentText())).getUnformattedComponentText());
        list.add(new TextComponentTranslation("text.oredowsing.tooltip.1",
                1 + 2 * getSquareRadius(stack)).getUnformattedComponentText());
        if (allowTargetChange) {
            list.add(new TextComponentTranslation("text.oredowsing.tooltip.2").getUnformattedComponentText());
        }
        if (isChargeable) {
            list.add(new TextComponentTranslation("text.oredowsing.tooltip.3").getUnformattedComponentText());
        }
        if (this.canUpgrade(stack, 1)) {
            list.add(new TextComponentTranslation(
                    "text.oredowsing.tooltip.4." + (itemsPerUpgrade == 1 ? "s" : "p"),
                    itemsPerUpgrade,
                    upgradeItemStack.getDisplayName()).getUnformattedComponentText());
        }
    }
    ...
    public void setTarget(ItemStack stack, IBlockState targetBlockState, EntityPlayer player) {
        if (!allowTargetChange) {
            if (player != null) {
                player.sendMessage(new TextComponentTranslation("text.oredowsing.change_target.no"));
            }
            return;
        }
    ...
    private void forceSetTarget(ItemStack stack, int targetBlockId, int targetBlockMeta, EntityPlayer player) {
        ...
        if (player != null) {
            player.sendMessage(new TextComponentTranslation("text.oredowsing.change_target.yes",
                    (targetBlockId == 0
                            ? new TextComponentTranslation("text.oredowsing.all_ores")
